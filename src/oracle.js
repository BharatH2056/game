...
export class Oracle {
    ...
    /**
     * Recursively predicts a sequence of likely future states.
     * @param {string} startRoom
     * @param {number} depth
     */
    predictSequence(startRoom, depth = 2) {
        const sequence = [];
        let currentRoom = startRoom;

        for (let i = 0; i < depth; i++) {
            const prediction = this.predictNext(currentRoom);
            if (!prediction || prediction.confidence < 0.5) break;
