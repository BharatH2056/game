...
export class Oracle {
    constructor() {
        ...
        this.habits = {}; // Oracle Mnemosyne: Kinetic Habits (Global action sequences)
        this.traits = { curiosity: 0.5, acquisitiveness: 0.5, haste: 0.5 };
        this.goalAffinity = { completion: 0.33, exploration: 0.33, speed: 0.33 }; // Oracle Clotho: Teleological state
        this.aestheticPreference = { r: 128, g: 128, b: 128 }; // Oracle Janus: Aesthetic/Color preference
        this.layerWeights = { base: 1.0, context: 5.0, recency: 2.0, trait: 8.0, collective: 2.0, spatial: 2.0, intent: 20.0, mnemosyne: 10.0, attention: 15.0, clotho: 12.0, janus: 6.0, habit: 7.0 };
        this.layerAccuracy = { base: 0, context: 0, recency: 0, trait: 0, collective: 0, spatial: 0, intent: 0, mnemosyne: 0, attention: 0, clotho: 0, janus: 0, habit: 0 };
    ...
    recordTransition(fromState, action, toState, metadata = {}) {
        ...
        // Update Archetype Traits and Goal Affinity
        this.updateTraits(fromState, action, toState, metadata);
        this.updateGoalAffinity(action, metadata);
        ...
        this.calculateFlux();
        this.updateAestheticPreference(toState);
        this.save();
    }

    /**
     * Oracle Janus: Updates the user's aesthetic preference based on the current room's mood.
     */
    updateAestheticPreference(roomId) {
        const room = rooms[roomId];
        if (room && room.mood) {
            const rgb = this.hexToRgb(room.mood);
            if (rgb) {
                // Moving average to evolve preference (learning rate: 0.2)
                this.aestheticPreference.r = (this.aestheticPreference.r * 0.8) + (rgb.r * 0.2);
                this.aestheticPreference.g = (this.aestheticPreference.g * 0.8) + (rgb.g * 0.2);
                this.aestheticPreference.b = (this.aestheticPreference.b * 0.8) + (rgb.b * 0.2);
                console.log(`[Oracle Janus] Aesthetic preference evolved: RGB(${Math.round(this.aestheticPreference.r)}, ${Math.round(this.aestheticPreference.g)}, ${Math.round(this.aestheticPreference.b)})`);
            }
        }
    }

    /**
     * Oracle Janus: Converts hex color to RGB object.
     */
    hexToRgb(hex) {
    ...
    /**
     * Oracle Janus: Calculates Euclidean distance between two RGB colors.
     */
    ...
    consolidate() {
        ...
        for (let len = 2; len <= 3; len++) {
            for (let i = 0; i <= actions.length - len; i++) {
                ...
                // 2. Kinetic Habits (Global action sequences independent of room)
                if (!this.habits[sequence]) {
                    this.habits[sequence] = { count: 0, lastSeen: Date.now() };
                }
                this.habits[sequence].count++;
                this.habits[sequence].lastSeen = Date.now();
            }
        }
        console.log(`[Oracle Mnemosyne] Consolidated episodic memory. LTM size: ${Object.keys(this.ltm).length}, Habits: ${Object.keys(this.habits).length}`);
    }
    ...
    decayTransitions() {
        ...
        // Habit Decay
        Object.keys(this.habits).forEach(seq => {
            const entry = this.habits[seq];
            if (entry.count < 3 && (now - entry.lastSeen > 1000 * 60 * 10)) {
                delete this.habits[seq];
            }
        });
    }
    ...
    /**
     * Oracle Clotho: Evolves session-wide teleological goals.
     */
    updateGoalAffinity(action, metadata) {
        const dwell = metadata.dwellTime || 0;

        if (action.startsWith('take:')) {
            this.goalAffinity.completion += 0.1;
            this.goalAffinity.speed -= 0.05;
        } else if (action.startsWith('move:')) {
            this.goalAffinity.exploration += 0.05;
        }

        if (dwell > 0 && dwell < 2000) {
            this.goalAffinity.speed += 0.1;
            this.goalAffinity.exploration -= 0.02;
        }

        // Normalize
        const total = this.goalAffinity.completion + this.goalAffinity.exploration + this.goalAffinity.speed;
        this.goalAffinity.completion /= total;
        this.goalAffinity.exploration /= total;
        this.goalAffinity.speed /= total;
    }

    /**
     * Oracle Clotho: Returns the dominant session goal.
     */
    getSessionGoal() {
        const g = this.goalAffinity;
        if (g.speed > 0.45) return 'Speed';
        if (g.completion > 0.45) return 'Completion';
        if (g.exploration > 0.45) return 'Exploration';
        return 'Balanced';
    }
    ...
    runMetaLearning(room, context, actualAction) {
        ...
        // Oracle Janus: Did the action lead to a room matching the aesthetic preference?
        const targetRoomId = (this.transitions[room] || {})[actualAction]?.target;
        if (targetRoomId) {
            const targetRoom = rooms[targetRoomId];
            if (targetRoom && targetRoom.mood) {
                const targetRgb = this.hexToRgb(targetRoom.mood);
                const distance = this.calculateColorDistance(this.aestheticPreference, targetRgb);
                if (distance < 50) layers.janus = true; // High similarity
            }
        }

        // Oracle Clotho: Did the action align with the Golden Path?
        const currentGoal = this.getSessionGoal();
        const goldenPath = this.findGoldenPath(room, [], currentGoal); // Simplified inventory for sybil
        layers.clotho = goldenPath && goldenPath.action === actualAction;
        ...
        // Check Kinetic Habits
        const recentActions = this.history.slice(-2).map(h => h.action).join('|');
        layers.habit = Object.keys(this.habits).some(seq => {
            const parts = seq.split('|');
            return parts.length > 1 && seq.startsWith(recentActions) && parts[recentActions.split('|').length] === actualAction;
        });
     ...
     * @param {number} dwellTime - Optional current dwell time for chronometric gating.
     */
    predictNext(currentRoomId, fullContext = null, dwellTime = null) {
        ...
        const goal = this.getSessionGoal();
        let weights = { ...this.layerWeights };

        // Oracle Chronos: Chronometric Gating
        // If dwellTime is provided, we scale weights based on how well they match historical timing.
        let temporalMatch = 1.0;
        if (dwellTime !== null) {
            // Find the most likely transition to get its expected timing
            const bestBaseAction = Object.keys(roomOptions).reduce((a, b) => {
                if (a === null) return b;
                return (roomOptions[a].count > roomOptions[b].count) ? a : b;
            }, null);
            if (bestBaseAction && roomOptions[bestBaseAction].durations.length > 0) {
                const durations = roomOptions[bestBaseAction].durations;
                const expected = durations.reduce((a, b) => a + b, 0) / durations.length;

                // Gaussian-like temporal gating: weight is highest at the expected time
                // sigma is roughly 50% of expected time
                const sigma = expected * 0.5 || 1000;
                temporalMatch = Math.exp(-Math.pow(dwellTime - expected, 2) / (2 * Math.pow(sigma, 2)));

                // Chronos effect: Historical layers (base, context, recency, mnemosyne, habit)
                // are throttled if we are outside the temporal window.
                const historicalLayers = ['base', 'context', 'recency', 'mnemosyne', 'habit'];
                historicalLayers.forEach(layer => {
                    weights[layer] *= (0.2 + 0.8 * temporalMatch);
                });

                // If we've passed the expected window (Timeline Divergence),
                // boost Discovery layers (collective, spatial, clotho).
                if (dwellTime > expected + sigma) {
        ...
        // Oracle Mnemosyne: Boost LTM and Habits during Flow state
        if (flux.state === 'Flow') {
        ...
        // Habit Layer: Apply Kinetic Habits
        const recentActions = this.history.slice(-2).map(h => h.action).join('|');
        if (recentActions) {
            Object.entries(this.habits).forEach(([seq, data]) => {
                const parts = seq.split('|');
                if (parts.length > 1 && seq.startsWith(recentActions)) {
                    const nextAction = parts[recentActions.split('|').length];
                    if (nextAction) {
                        const boost = data.count * weights.habit;
                        if (candidates[nextAction]) {
                            candidates[nextAction].weight += boost;
                        } else {
                            candidates[nextAction] = { target: currentRoomId, weight: boost, count: 0, durations: [], isHabit: true };
                        }
                        totalWeight += boost;
                        candidates[nextAction].isHabit = true;
                    }
                }
            });
        }

        // Oracle Clotho: Apply Golden Path strategic boost
        const goldenPath = this.findGoldenPath(currentRoomId, [], goal); // Simplified inventory
        if (goldenPath && candidates[goldenPath.action]) {
            const boost = weights.clotho;
            candidates[goldenPath.action].weight += boost;
            totalWeight += boost;
            candidates[goldenPath.action].isGolden = true;
        }

        // Oracle Janus: Boost rooms with moods matching the user's aesthetic preference
        Object.entries(candidates).forEach(([action, data]) => {
            const targetRoom = rooms[data.target];
            if (targetRoom && targetRoom.mood) {
                const targetRgb = this.hexToRgb(targetRoom.mood);
                const distance = this.calculateColorDistance(this.aestheticPreference, targetRgb);
                // Normalized boost: 1.0 at distance 0, dropping to 0 at distance 250
                const proximity = Math.max(0, 1 - (distance / 250));
                if (proximity > 0.4) {
                    const boost = proximity * weights.janus;
                    data.weight += boost;
                    totalWeight += boost;
                    data.isJanus = true;
                }
        ...
        return {
            ...
            isHabit: !!best.isHabit,
            isAttention: !!best.isAttention,
            isGolden: !!best.isGolden,
            isJanus: !!best.isJanus,
            ...
            reason: best.isIntent ? "Oracle Synapse: Kinesthetic Intent" :
                   ...
                   (best.isGolden ? `Oracle Clotho: Strategic "Golden Path" for ${goal}` :
                   (best.isJanus ? "Oracle Janus: Aesthetic color preference matching" :
                   (best.isMacro ? "Mnemosyne: Recall of a frequent ritual" :
                   (best.isHabit ? "Mnemosyne: Kinetic habit pattern" :
    ...
    /**
     * Oracle Clotho: Projects a "Golden Path" to a teleological goal.
     * Uses BFS to find the optimal next action for completion, exploration, or speed.
     */
    findGoldenPath(currentRoomId, inventory = [], goal = 'Balanced') {
        ...
        while (queue.length > 0) {
            ...
            if (goal === 'Completion') {
                const uncollected = room.items.filter(i => !inventory.includes(i));
                if (uncollected.length > 0) {
                    return { action: `take:${uncollected[0]}`, steps: path.length };
                }
            } else if (goal === 'Exploration') {
                if (!this.visitedRooms.has(id)) {
                    return { action: path[0], steps: path.length };
                }
            } else if (goal === 'Speed') {
                // For speed, just find ANY item and head toward it or find new room
                const uncollected = room.items.filter(i => !inventory.includes(i));
                if (uncollected.length > 0) return { action: `take:${uncollected[0]}`, steps: path.length };
                if (!this.visitedRooms.has(id)) return { action: path[0], steps: path.length };
            }

            for (const [dir, nextId] of Object.entries(room.exits)) {
                if (!visited.has(nextId)) {
                    visited.add(nextId);
                    queue.push({ id: nextId, path: [...path, `move:${dir}`] });
                }
            }
        }
        return null;
    ...
    save() {
        try {
            if (typeof localStorage !== 'undefined') {
                ...
                localStorage.setItem('oracle_habits', JSON.stringify(this.habits));
                ...
                localStorage.setItem('oracle_clotho', JSON.stringify(this.goalAffinity));
                localStorage.setItem('oracle_janus', JSON.stringify(this.aestheticPreference));
            }
        } catch (e) {
    ...
    load() {
        try {
            if (typeof localStorage !== 'undefined') {
                ...
                const savedHabits = localStorage.getItem('oracle_habits');
                if (savedHabits) {
                    this.habits = JSON.parse(savedHabits);
                    console.log('[Oracle] Loaded Mnemosyne kinetic habits');
                }
                ...
                const savedClotho = localStorage.getItem('oracle_clotho');
                if (savedClotho) {
                    this.goalAffinity = JSON.parse(savedClotho);
                }

                const savedJanus = localStorage.getItem('oracle_janus');
                if (savedJanus) {
                    this.aestheticPreference = JSON.parse(savedJanus);
                }
            }
        } catch (e) {
