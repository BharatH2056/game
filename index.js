...
class Game {
    ...
    updateTemporalUI(inference) {
        ...
        const sybilIcon = dominantLayer === 'intent' ? '⚡' : (dominantLayer === 'mnemosyne' ? '🏛️' : (dominantLayer === 'clotho' ? '✨' : (dominantLayer === 'context' ? '🧠' : (dominantLayer === 'collective' ? '📡' : (dominantLayer === 'trait' ? '✨' : '👁️')))));
        ...
        window.addEventListener('oracle:prefetch', (e) => {
            ...
            buttons.forEach(btn => {
                ...
                if (actionKey === prediction.action) {
                    ...
                    btn.classList.remove('oracle-collective', 'oracle-intuitive', 'oracle-spatial', 'synapse-lock', 'guidance-aura', 'soma-lock', 'mnemosyne-recall');
                    ...
                    btn.classList.remove('oracle-hint', 'oracle-intuitive', 'oracle-collective', 'oracle-spatial', 'synapse-lock', 'guidance-aura', 'soma-lock', 'mnemosyne-recall', 'argus-focus');
                }
            });
    ...
    updateVision(prediction) {
        ...
        if (prediction && prediction.confidence > 0.6) {
            const targetRoom = rooms[prediction.predictedState];
            const label = prediction.isSomaAssisted ? 'Soma Assistance' : (prediction.isIntent ? 'Synapse' : (prediction.isAttention ? 'Argus' : (prediction.isGolden ? 'Golden Path' : (prediction.isMacro ? 'Mnemosyne' : (prediction.isGuidance ? 'Guidance' : (prediction.isCollective ? 'Collective Wisdom' : (prediction.isIntuitive ? 'Intuition' : 'Vision')))))));
            let html = `<div class="vision-next ${prediction.isCollective ? 'vision-collective' : ''} ${prediction.isIntent ? 'vision-synapse' : ''} ${prediction.isMacro ? 'vision-mnemosyne' : ''} ${prediction.isGuidance ? 'vision-guidance' : ''} ${prediction.isSomaAssisted ? 'vision-soma' : ''} ${prediction.isAttention ? 'vision-argus' : ''} ${prediction.isGolden ? 'vision-golden' : ''}"><span class="vision-label">${label}:</span> You sense ${targetRoom ? targetRoom.name : 'something'} ahead...</div>`;
