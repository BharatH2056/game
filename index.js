...
class Game {
    ...
    updateVision(prediction) {
        ...
        if (prediction && prediction.confidence > 0.6) {
            ...
            // Deep Vision: Try to see one more step ahead
            const sequence = this.oracle.predictSequence(prediction.predictedState, 1);
            if (sequence.length > 0) {
                const deepTarget = rooms[sequence[0].predictedState];
                html += `<div class="vision-deep"><span class="vision-label-deep">Beyond the horizon:</span> A faint glimmer of ${deepTarget ? deepTarget.name : 'the unknown'}</div>`;
            }
