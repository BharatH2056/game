...
class Game {
        ...
        window.addEventListener('oracle:prefetch', (e) => {
            ...
            buttons.forEach(btn => {
                ...
                if (actionKey === prediction.action) {
                    ...
                    btn.classList.remove('oracle-hint', 'oracle-intuitive', 'oracle-collective', 'oracle-spatial', 'synapse-lock', 'guidance-aura', 'soma-lock', 'mnemosyne-recall');
                }
            });
    ...
    updateVision(prediction) {
        ...
        if (prediction && prediction.confidence > 0.6) {
            const targetRoom = rooms[prediction.predictedState];
            const label = prediction.isSomaAssisted ? 'Soma Assistance' : (prediction.isIntent ? 'Synapse' : (prediction.isMacro ? 'Mnemosyne' : (prediction.isGuidance ? 'Guidance' : (prediction.isCollective ? 'Collective Wisdom' : (prediction.isIntuitive ? 'Intuition' : 'Vision')))));
