...
class Game {
    ...
    updateTemporalUI(inference) {
        ...
        const traits = this.oracle.traits;
        let archetype = "The Observer";
        if (traits.curiosity > 0.7) archetype = "The Explorer";
        if (traits.acquisitiveness > 0.7) archetype = "The Collector";
        if (traits.haste > 0.7) archetype = "The Speedrunner";
        if (traits.curiosity > 0.6 && traits.acquisitiveness > 0.6) archetype = "The Completionist";

        archetypeEl.innerText = archetype;

        // Update status with behavioral profile
        statusEl.innerText = `${profile} ${inference.isStagnant ? '(Stagnant)' : ''}`;
    ...
    setupOracleListeners() {
        window.addEventListener('oracle:prefetch', (e) => {
            ...
            logEntry.className = prediction.isIntuitive ? 'log-intuitive' : (prediction.isContextual ? 'log-contextual' : '');
