...
class Game {
    ...
    /**
     * Tracks mouse kinematics to feed the Oracle's Synapse layer.
     */
    ...
    updateTemporalUI(inference) {
        ...
        if (prediction && prediction.confidence > 0.4) {
            ...
            if (currentRoom && targetRoom) {
                const container = document.getElementById('game-container');
                // Mix current mood with predicted mood based on confidence
        ...
        window.addEventListener('oracle:prefetch', (e) => {
            ...
            buttons.forEach(btn => {
                ...
                if (actionKey === prediction.action) {
                    ...
                    btn.classList.remove('oracle-collective', 'oracle-intuitive', 'oracle-spatial', 'synapse-lock');
                    ...
                    btn.classList.remove('oracle-hint', 'oracle-intuitive', 'oracle-collective', 'oracle-spatial', 'synapse-lock');
                }
            });
    ...
    updateVision(prediction) {
        ...
        if (prediction && prediction.confidence > 0.6) {
            const targetRoom = rooms[prediction.predictedState];
            const label = prediction.isIntent ? 'Synapse' : (prediction.isCollective ? 'Collective Wisdom' : (prediction.isIntuitive ? 'Intuition' : 'Vision'));
            let html = `<div class="vision-next ${prediction.isCollective ? 'vision-collective' : ''} ${prediction.isIntent ? 'vision-synapse' : ''}"><span class="vision-label">${label}:</span> You sense ${targetRoom ? targetRoom.name : 'something'} ahead...</div>`;
