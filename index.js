...
class Game {
        ...
        window.addEventListener('oracle:prefetch', (e) => {
            ...
            buttons.forEach(btn => {
                ...
                if (actionKey === prediction.action) {
                    ...
                    btn.classList.remove('oracle-collective', 'oracle-intuitive', 'oracle-spatial', 'synapse-lock', 'guidance-aura', 'soma-lock');
                    ...
                    btn.classList.remove('oracle-hint', 'oracle-intuitive', 'oracle-collective', 'oracle-spatial', 'synapse-lock', 'guidance-aura', 'soma-lock');
                }
            });
    ...
    updateVision(prediction) {
        ...
        if (prediction && prediction.confidence > 0.6) {
            const targetRoom = rooms[prediction.predictedState];
            const label = prediction.isSomaAssisted ? 'Soma Assistance' : (prediction.isIntent ? 'Synapse' : (prediction.isGuidance ? 'Guidance' : (prediction.isCollective ? 'Collective Wisdom' : (prediction.isIntuitive ? 'Intuition' : 'Vision'))));
            let html = `<div class="vision-next ${prediction.isCollective ? 'vision-collective' : ''} ${prediction.isIntent ? 'vision-synapse' : ''} ${prediction.isGuidance ? 'vision-guidance' : ''} ${prediction.isSomaAssisted ? 'vision-soma' : ''}"><span class="vision-label">${label}:</span> You sense ${targetRoom ? targetRoom.name : 'something'} ahead...</div>`;
